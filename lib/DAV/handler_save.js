...
(function() {
     ...
     */
    this.walkPath = function(path, nodes, nodesPath, directoriesToGo, optFunc) { // call by reference auf nodes
		var self = this;
		
		if (path.indexOf(".svn") >= 0) // TODO: find a way to handle .svn directories better. See below.
			return
		if (directoriesToGo.indexOf(path) < 0)  
			directoriesToGo.push(path) // to cope with the asynchrosous callbacks ...
			...
			if (parentNode.hasFeature(jsDAV.__ICOLLECTION__)) {
				parentNode.getChildren(function(err, cNodes) {
					...
					for (var i = 0, l = cNodes.length; i < l; ++i) {
						...
						if (cNodes[i].hasFeature(jsDAV.__ICOLLECTION__)) {
							if (directoriesToGo.indexOf(childPath) < 0 && childPath.indexOf("svn") < 0) // TODO: find a way to handle .svn directories better. See above.
								directoriesToGo.push(childPath)
							self.walkPath(childPath, nodes, nodesPath, directoriesToGo, optFunc);
						}	
					}
					directoriesToGo.splice(directoriesToGo.indexOf(path), 1);
					if (directoriesToGo.length == 0)
						optFunc && optFunc(nodes, nodesPath);
				});
			}
		})
    ...
    this.getPropertiesForPath = function(path, propertyNames, depth, cbgetpropspath) {
        ...
        if (depth !== 0 && depth !== exports.DEPTH_INFINITY)
            depth = 1;
        ...
        this.server.tree.getNodeForPath(path, function(err, parentNode) {
			...
			if (depth == exports.DEPTH_INFINITY && parentNode.hasFeature(jsDAV.__ICOLLECTION__)) {
				self.walkPath(path, nodes, nodesPath, [], function (nodes, nodesPath) {					
					afterGetChildren(nodes, nodesPath);
				})
			}
			else if ((depth == 1) && parentNode.hasFeature(jsDAV.__ICOLLECTION__)) {
				parentNode.getChildren(function(err, cNodes) {
					if (!Util.empty(err))
						return cbgetpropspath(err);
					for (var i = 0, l = cNodes.length; i < l; ++i) {
